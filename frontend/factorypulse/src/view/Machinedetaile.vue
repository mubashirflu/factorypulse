...
async function fetchInitialData() {
  try {
    ...
    console.error('Failed to fetch initial reading data', err)
  }
}
...
function connectWebSocket() {
  ...
  socket.onmessage = (event) => {
    ...
    // Sirf isi machine ka data process karo
    if (message.type === 'reading' && message.machine_id === Number(machineId)) {
      ...
      latest.value = newReading
      history.value.push(newReading)
      if (history.value.length > 30) history.value.shift() // sirf last 30 rakho
...
<template>
  <div class="min-h-screen bg-base p-8">
    ...
    <div class="flex items-center justify-between mb-8">
      <h2 class="font-display text-2xl font-semibold text-text-primary">Machine #{{ machineId }}</h2>
      <div class="flex items-center gap-2 font-mono text-xs text-text-muted">
        <span :class="['w-2 h-2 rounded-full', connected ? 'bg-live animate-pulse' : 'bg-critical']"></span>
        {{ connected ? 'Live · WebSocket connected' : 'Disconnected' }}
      </div>
    </div>
